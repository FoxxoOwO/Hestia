...
def seed_demo_data(db: Session):
    ...
    user_anna = db.query(User).filter(User.username == "anna").first()
    if not user_anna:
        user_anna = User(
            username="anna",
            display_name="Anna",
        ...
        db.add(user_anna)
        db.commit()
        db.refresh(user_anna)

    user_petr = db.query(User).filter(User.username == "petr").first()
    if not user_petr:
        user_petr = User(
            username="petr",
            display_name="Petr",
...
seed_initial_data = seed_demo_data
